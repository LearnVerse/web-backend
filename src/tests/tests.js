...
(async () => {
  try {
    ...
    const partyResponse = await axios.get('http://localhost:9090/server/createParty', { params: { name, game, numPlayers } });
    ...
    const student1Response = await axios.get('http://localhost:9090/server/joinPartyAsStudent', { params: { name: 'Student1', partyCode } });
    ...
    // Attempt to add Student1 to party; EXPECT: IP address 100.100.100.100
    const student2Response = await axios.get('http://localhost:9090/server/joinPartyAsStudent', { params: { name: 'Student2', partyCode } });
    if (student2Response.data.status === 200) console.log(`SUCCESS! Student2 added to server ${student2Response.data.data}`);
    else throw new Error('ERROR! Couldn\'t add Student1 to party');

    // Attempt to add Student1 to party; EXPECT: IP address 200.200.200.200
    const student3Response = await axios.get('http://localhost:9090/server/joinPartyAsStudent', { params: { name: 'Student3', partyCode } });
    if (student3Response.data.status === 200) console.log(`SUCCESS! Student3 added to server ${student3Response.data.data}`);
    else throw new Error('ERROR! Couldn\'t add Student1 to party');

    // Attempt to add Student1 to party; EXPECT: IP address 200.200.200.200
    const student4Response = await axios.get('http://localhost:9090/server/joinPartyAsStudent', { params: { name: 'Student4', partyCode } });
